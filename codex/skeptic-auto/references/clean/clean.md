...
Script contract: generate `03_cleaning.py` for the current project and follow `../script-contract.md`. Include only the helpers needed for the visible artifacts and active cycle evidence. Cycle R adds `clean_data(...)`, `preprocess_data(...)`, and the reproducibility validator; Cycle S adds stability or transfer diagnostics only when required.

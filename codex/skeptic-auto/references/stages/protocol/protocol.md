...
2. Recover prior stage state:
   - Cycle A:
     ...
     - Load `references/routes/protocol/{route}.md` once and keep it in memory for the rest of the stage. If the route file is missing, stop.
   ...
   - First cycle entered in a fresh session (not Cycle A), or first cycle after a backtrack reopens the stage: read `02_protocol.yaml` and reload `references/routes/protocol/{route}.md` once.
...
Agent(
  ...
  4. references/routes/protocol/{route}.md

...
Every numeric pass/fail threshold in the analyze stage (perturbation-plan accuracy metric, perturbation axis scope, challenger divergence criterion, assumption thresholds in Cycle B, comparison-table material-difference threshold in Cycle D) must carry a `derivation:` field in the canonical YAML with mode one of {empirical, declared_external_standard, upstream_ratio, user_specified}. Mode `agent_chosen` is forbidden. The threshold value and its derivation are locked at contract lock (Cycle A) and may not be re-selected after execution begins. The `metric_graph:` (authored once in Cycle A under `contract.metric_graph`) is the source of truth for which metrics are expected to change under a perturbation; it is referenced read-only by all later cycles and must not be re-authored outside Cycle A. See core-principles.md section "Threshold hygiene" for the universal rule.
...
provenance:                         # immutable audit facts only
  upstream_artifacts: {}            # A2: {path: {expected_sha256, observed_sha256, match: bool}} -- hash value not propagated; only match verdict recorded here
  # A3 visibility derivation: computed at cycle-start from 02_protocol.yaml; stored as visible_artifacts [{name, access_level}] under visibility; not re-documented here
  visibility:
...
contract:                           # locked in Cycle A; may be amended only under A07 policy
  ...
  accuracy_metric:                  # when route overlay requires; carries derivation: {mode, source, value, approved_by, approved_at}; mode agent_chosen forbidden
  perturbation_plan: {}             # {axes: [], types: [], scope}; any numeric threshold per axis carries derivation: {mode, source, computation, value, approved_by, approved_at}; mode agent_chosen forbidden
  metric_graph: {}                  # authored once at contract lock; referenced read-only by all later perturbation cycles (analyze D, evaluate B, clean S)
                                    # schema: {<raw_variable>: {inputs: []}, <derived_metric>: {inputs: [<raw_variable>, ...], dependency_type: ratio|arithmetic_complement|formula_parameter|shared_feature|indirect}}
                                    # expected exceedance set for a perturbation to X = transitive closure of metrics whose inputs chain reaches X
...
Agent(
  ...
  matching the schema in analyze.md (Research log section). Do not inline URLs
